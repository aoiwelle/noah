...
When you need to gather structured information from the user:
Call `ui_spa` with `action.type = GATHER` and an optional `action.gather_schema` (JSON Schema describing the form fields).
...
- IMPORTANT: Always call knowledge tools BEFORE your final UI tool call, never in the same turn as your concluding `ui_*` call. Run tools first, then emit the UI call.
